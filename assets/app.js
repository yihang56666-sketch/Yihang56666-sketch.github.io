  ...
  const posts = [
    {
      ...
      summary: "一个通用的 Codex Skill，用来判断何时分派任务、选择边界清晰的只读角色、处理失败，并汇总原生子智能体的结果；已经作为公开仓库发布。",
      ...
      sections: [
        ...
        },
        {
          ...
          bullets: [
            "更轻：无自定义运行时、API 客户端、状态数据库或仪表盘，维护边界集中在文档与契约测试。",
            "更安全：要求记录 Git 基线和内容指纹，子智能体只读，主智能体仍是唯一写入者。",
            "更可审计：子智能体必须返回文件、行号、命令输出级别证据，主智能体统一核验后才完成交付。",
            "更节制的并行：默认只使用 1 到 3 个子智能体，不默认“越多越强”。",
            "失败语义接近原生能力：等待超时不等于失败，只有终态失败或证据不足才做至多一次窄范围替补。"
          ],
          table: {
            ...
            rows: [
              ...
              ["失败处理", "通用重试 / 状态机", "超时不等于失败；终态失败才有限替补"],
              ["第三方动作", "取决于框架和开发者配置", "所有子智能体明确只读，主智能体握有写入权"]
            ]
          }
        }
      ]
      ...
      detail: [
        ...
        },
        {
          ...
          bullets: [
            ...
            "ruff 与 mypy 全绿，最近一轮回归为 1013 passed / 12 skipped；真实板卡日流程有独立 runbook 记录。"
          ]
        },
        {
          heading: "仓库分层",
