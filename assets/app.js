  ...
  const posts = [
    {
      ...
      sections: [
        ...
        },
        {
          ...
          paragraphs: [
            "开源多智能体框架通常把重点放在执行引擎：角色循环、工具调用、通信、进度状态和模型后端。这个 Skill 不做这些，执行层直接复用 Codex 原生的 spawn_agent，所以没有自定义运行时、API 客户端、仪表盘或状态数据库。",
            "它补的是执行之上容易被忽略的三层：要不要分派、怎么给每个子智能体画边界，以及怎么验证和汇总结果。"
          ],
          bullets: [
            "结构更轻薄：默认只使用 1 到 3 个子智能体，不默认“越多越强”。",
            "工作区更安全：要求记录 Git 基线和内容指纹，子智能体只读，主智能体仍是唯一写入者。",
            "失败语义更贴近原生能力：等待超时不等于失败，只有终态失败或证据不足才做至多一次窄范围替补。",
            "优势不在引擎，而在审计与交付：子智能体必须返回文件、行号、命令输出级别证据，主智能体统一核验后完成最终交付。"
          ]
        }
      ]
    },
